...
class TikTokenizerPPLCalc(object):
    ...
    def get_bbs_ll(self, tokens, ll):
        ...
        for idx, token in enumerate(tokens):
            """
                If the token is a byte-encoded token (e.g., bytes:\x), the method counts how many byte sequences are present and adds a placeholder
                log-likelihood for each byte (since GPT models output tokens, not bytes).
